...
El método de las dos fases también asocia al problema que queremos resolver otro problema, al que llamaremos \tbi[problema!penalizado]{problema penalizado}, cuyas restricciones son exactamente las mismas a las del problema artificial. Lo único que los diferencia es la función objetivo, que en este caso es
\begin{equation*}
	u:\equiv z+Mw
\end{equation*}
...
\end{obs}
\begin{obs}[Sutilezas]
	Evidentemente el problema penalizado es factible, por el mismo motivo que lo es el problema artificial. No obstante, y al contrario de lo que sucedía con el problema artificial, el problema penalizado puede estar no acotado. En efecto, basta que el problema original sea no acotado.
\end{obs}
...
\begin{proof}
	...
	\begin{itemize}
		\item En caso de detectar la optimalidad (digamos para un punto $(x^*|x_a)^t$), es claro que el problema es infactible, ya que de existir una solución factible $\overline{x}$ del problema original (sin pérdida de generalidad un punto extremo) se tendría que $c^t\overline{x}\leq c^tx^*+Mw(x^*|x_a)^t$, siempre y cuando $M$ sea adecuadamente grande (especificaremos luego).
...
Presentamos a continuación otra regla de prevención de ciclos, debida a Robert G. Bland ($1977$), y que, como es evidente, es conocida como \tbi[regla!de Bland]{regla de Bland}. Esta regla es muy sencilla y constituye un criterio de entrada y salida a la base.
